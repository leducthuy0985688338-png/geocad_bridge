...
class CoordinateTransformService {
  ...
  bool isValidWgs84({required double longitude, required double latitude}) {
  ...
  bool isValidUtmLatitude(double latitude) {
    return latitude.isFinite && latitude >= -80.0 && latitude <= 84.0;
  }
  ...
  GeographicCoordinate utmToWgs84({
    ...
    _validateAltitude(altitude);

    if (!isValidUtm(easting: easting, northing: northing, zone: zone)) {
    ...
    final eccPrimeSquared = _eccSquared / (1.0 - _eccSquared);
    ...
    final mu =
        ...
        (_a *
            (1.0 -
                ...
                5.0 * _eccSquared * _eccSquared * _eccSquared / 256.0));
    ...
    final n1 =
        _a /
        math.sqrt(1.0 - _eccSquared * math.sin(phi1Rad) * math.sin(phi1Rad));

    final t1 = math.tan(phi1Rad) * math.tan(phi1Rad);

    final c1 = eccPrimeSquared * math.cos(phi1Rad) * math.cos(phi1Rad);

    final r1 =
        _a *
        (1.0 - _eccSquared) /
        math.pow(
          1.0 - _eccSquared * math.sin(phi1Rad) * math.sin(phi1Rad),
          1.5,
        );
    ...
    final longOrigin = (zone - 1) * 6 - 180 + 3;

    latitude = _radToDeg(latitude);
    longitude = longOrigin + _radToDeg(longitude);

    if (!isValidWgs84(longitude: longitude, latitude: latitude)) {
      throw StateError('Kết quả WGS84 nằm ngoài phạm vi hợp lệ.');
    }
  ...
  MapCoordinate wgs84ToUtm({
    ...
    if (!isValidUtmLatitude(latitude)) {
      throw ArgumentError('Latitude UTM phải nằm trong [-80, 84].');
    }
    ...
    if (!_isHemisphereCompatible(latitude, hemisphere)) {
      throw ArgumentError('Bán cầu UTM không phù hợp với latitude.');
    }
    ...
    final n =
        _a / math.sqrt(1.0 - _eccSquared * math.sin(latRad) * math.sin(latRad));

    final t = math.tan(latRad) * math.tan(latRad);

    final c = eccPrimeSquared * math.cos(latRad) * math.cos(latRad);

    final aTerm = math.cos(latRad) * (longRad - longOriginRad);
    ...
    final m =
        ...
        ((1.0 -
            ...
            (3.0 * _eccSquared / 8.0 +
                    ...
                    45.0 * _eccSquared * _eccSquared * _eccSquared / 1024.0) *
                math.sin(2.0 * latRad) +
            (15.0 * _eccSquared * _eccSquared / 256.0 +
                    45.0 * _eccSquared * _eccSquared * _eccSquared / 1024.0) *
                math.sin(4.0 * latRad) -
            (35.0 * _eccSquared * _eccSquared * _eccSquared / 3072.0) *
                math.sin(6.0 * latRad));
    ...
    if (!isValidUtm(easting: easting, northing: northing, zone: zone)) {
      throw StateError('Kết quả UTM nằm ngoài phạm vi hợp lệ.');
    }

    return MapCoordinate(x: easting, y: northing, z: altitude);
  }
  ...
  GeographicCoordinate toWgs84({
    ...
    if (!sourceCrs.isValid) {
      throw ArgumentError('Hệ tọa độ nguồn không hợp lệ.');
    }

    _validateAltitude(coordinate.z);

    switch (sourceCrs.type) {
      case CoordinateReferenceSystemType.wgs84:
        if (!isValidWgs84(longitude: coordinate.x, latitude: coordinate.y)) {
          throw ArgumentError('Tọa độ WGS84 không hợp lệ.');
        }
  ...
  MapCoordinate fromWgs84({
    ...
    if (!targetCrs.isValid) {
      throw ArgumentError('Hệ tọa độ đích không hợp lệ.');
    }

    _validateAltitude(coordinate.altitude);
  ...
  int suggestedUtmZone(double longitude) {
    if (!longitude.isFinite || longitude < -180.0 || longitude > 180.0) {
      throw ArgumentError('Kinh độ phải nằm trong [-180, 180].');
    }
  ...
  UtmHemisphere suggestedHemisphere(double latitude) {
    if (!isValidUtmLatitude(latitude)) {
      throw ArgumentError('Latitude UTM phải nằm trong [-80, 84].');
    }

    return latitude >= 0 ? UtmHemisphere.north : UtmHemisphere.south;
  }

  bool _isHemisphereCompatible(double latitude, UtmHemisphere hemisphere) {
  ...
  void _validateAltitude(double? altitude) {
    if (altitude != null && !altitude.isFinite) {
      throw ArgumentError('Altitude/Z phải là số hữu hạn.');
    }
  }
